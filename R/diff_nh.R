...
#' @param options  Time window for velocity computation in \emph{milliseconds}
#'
...
diff_nh <- function(x, y,  trial, sample_rate, options){
  ...
  data.frame(trial = trial,
    ...
    dplyr::mutate(x = (.data$x - lag(x)) / delta_t_s,
                  y = (.data$y - lag(y)) / delta_t_s,
                  amp = sqrt(.data$velx^2 + .data$vely^2)) %>%
